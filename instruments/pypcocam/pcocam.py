...
class PcoCamera(Camera):
    ...
    def set_roi(self, width: int, height: int) -> None:
        ...
        requirement (confirmed empirically: an off-center window is accepted by
        `sdk.set_roi()` but rejected by `arm()` with `CameraException 0x8003103e`
        "ROI setting is wrong" -- there is no field in this SDK's description dict
        to detect the requirement ahead of time, so centering unconditionally is
        the only way to guarantee `arm()` succeeds).
